...
app.get('/api/extrato', checkAuth, (req, res) => {
    db.all("SELECT t.id, t.placa_registrada, v.modelo, c.nome as cliente, s.nome as servico, t.valor_cobrado, t.status, t.forma_pagamento, t.data_hora FROM transacoes t JOIN clientes c ON t.cliente_id = c.id JOIN servicos s ON t.servico_id = s.id LEFT JOIN veiculos v ON t.veiculo_id = v.id ORDER BY t.data_hora DESC LIMIT 100", (err, rows) => res.json(rows||[]));
});
