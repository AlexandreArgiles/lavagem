...
async function loadExt(){const r=await fetch(`${API}/extrato`);const d=await r.json();const t=document.querySelector('#tabela-extrato tbody');t.innerHTML='';d.forEach(x=>t.innerHTML+=`<tr><td>${new Date(x.data_hora).toLocaleDateString()}</td><td>${x.placa_registrada}<br><small>${x.modelo||''}</small></td><td>${x.servico}</td><td>${fmt(x.valor_cobrado)}</td><td>${x.status==='CONCLUIDO'?'✅':'⏳'}</td><td><button class="btn-action" onclick="abrirModalImpressao({nome:'${x.cliente}',placa:'${x.placa_registrada}',modelo:'${x.modelo||''}',servico:'${x.servico}',valor:${x.valor_cobrado},pgto:'${x.forma_pagamento}'})">🖨️</button><button class="btn-action btn-edit admin-only" onclick="abrirModal('transacao',${x.id},'',${x.valor_cobrado},'${x.forma_pagamento}')">✏️</button><button class="btn-action btn-delete admin-only" onclick="delItem('transacoes',${x.id},loadExt)">🗑️</button></td></tr>`);applyRole()}
async function loadDesp(){const r=await fetch(`${API}/despesas`);const d=await r.json();const t=document.querySelector('#tabela-despesas tbody');t.innerHTML='';d.forEach(x=>t.innerHTML+=`<tr><td>${new Date(x.data_despesa).toLocaleDateString()}</td><td>${x.descricao}</td><td>-${fmt(x.valor)}</td><td><button onclick="delItem('despesas',${x.id},loadDesp)">X</button></td></tr>`)}
...
async function loadServ(){const r=await fetch(`${API}/servicos`);const d=await r.json();const t=document.querySelector('#tabela-servicos tbody');t.innerHTML='';d.forEach(x=>t.innerHTML+=`<tr><td>${x.nome}</td><td>${fmt(x.preco)}</td><td><button onclick="delItem('servicos',${x.id},loadServ)">X</button></td></tr>`)}
